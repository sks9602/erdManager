    Ext.define('UserUI.view.shared.CheckboxGroupingFeature', {
      ...
      constructor: function(config) {
        config.groupHeaderTpl = ['<input class="' + this.targetCls + '" {[values.record.get("' + this.checkDataIndex + '") ? "checked" : ""]} type="checkbox"> {name:this.formatName}',
                                                        {
        ...
        if (event && grid) {
          ...
          if (target && store && groupRecord) {
           ...
           store.each(function(recordColumn){
                if( group == recordColumn.get(groupField)) {
                    console.log( groupField, recordColumn.get(groupField) )
            ...
            if( checked ) {
               ...
               grid.getSelectionModel().deselect( selections ); 
            }
          } else {
            this.callParent(arguments);
          }

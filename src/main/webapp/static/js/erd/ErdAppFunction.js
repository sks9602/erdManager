...
class  ErdAppFunction{
	...
	static deleteTableWindow(drawDataLoad, type) {
		...
		if(Ext.getCmp("deleteTableOrRelation") != null) {
			Ext.getCmp("deleteTableOrRelation").show();
		} else {
				 ...
				 buttons : [
					 { text: '삭제', handler : function() {
						...
						if( Ext.getCmp('delType_ALL').checked ) {
							if( type == 'deleteSelectedObjects' ) {
								 cfrmMsg = "선택된 " + size+"개의 테이블을 프로젝트에서 삭제하시겠습니까?";
							}else if( type == 'deleteEntity' ) {
								 cfrmMsg = "선택된 " + table_nm+"["+entity_nm+"]테이블을 프로젝트에서 삭제하시겠습니까?";
							}
						} else {
							if( type == 'deleteSelectedObjects' ) {
								 cfrmMsg = "선택된 " + size+"개의 테이블을 업무영역에서 삭제하시겠습니까?";
							}else if( type == 'deleteEntity' ) {
								 cfrmMsg = "선택된 " + table_nm+"["+entity_nm+"]테이블을 업무영역에서 삭제하시겠습니까?";
							}
						}
									
						 Ext.Msg.confirm('확인', cfrmMsg, function(btn) {
										...
										drawDataLoad.getSelectedTables()["ENTITY_IDS"].forEach(function(entity_id, entity_id2, set) {
											...
											if( result.success ){
												if( result.SUBJECT_ENTITY_DELETE_YN == "Y" ) {
													if( table ) {
														table.deleteTable( Ext.getCmp('delType_ALL').checked );
														drawDataLoad.updateEntityDatas(entity_id, "USE_YN", "N", delType == 'deleteTableInProject');
														/* TODO - 테이블 삭제 후 다시 추가
														if( delType == 'deleteTable') {
															for( var i=0 ; i<drawDataLoad.entityDatas.length; i++ ) {
																if( drawDataLoad.entityDatas[i].ENTITY_ID == entity_id ) {
																	delete drawDataLoad.entityDatas[i];
																}
															}
														}
														*/ 
														// 데이터 삭제,
														drawDataLoad.deleteTableOnSubjectAreaDatas(drawDataLoad.getSelectedTables()["SUBJECT_ID"], entity_id);
														drawDataLoad.removeDrawedTable(drawDataLoad.getSelectedTables()["SUBJECT_ID"] , entity_id);
													}
												} else {
										...
										if( result.success ){
											if( result.SUBJECT_ENTITY_DELETE_YN == "Y" ) {
												if( table ) {
													...
													*/
													drawDataLoad.removeDrawedTable(subject_id, entity_id);
												}
											} else {
												var pkInsertEntityList = result.PK_INSERT_ENTITY_LIST;
												var pkDeleteEntityList = result.PK_DELETE_ENTITY_LIST;
									...
									Ext.getCmp('addTableForm').submit({
										...
										success: function(form, action) {
										  ...
										  Ext.Msg.alert('성공', action.result.message, function() {
											  ...
											  */
											  var entity_tcd = Ext.getCmp("addTableWindow_ENTITY_TCD").getValue()
									...
									Ext.getCmp('editTableForm').submit({
										...
										success: function(form, action) {
										  ...
										  Ext.Msg.alert('성공', action.result.message, function() {
											  ...
											  */
											 var tabl_scd_nm = action.result.ENTITY_LIST[0].TABL_SCD_NM;
											  var entity_id = action.result.ENTITY_ID;
			  ...
			  items : [
				  {
						...
						items: [
								...
								listeners: {
									afterrender: function(field) {
										...
										if( Ext.getCmp("loginForm_CHECK_ID_SAVE").checked  ) {
											...
											field.setValue(getCookie('LOGIN_ID'));
										} 
										field.focus();
									},
									change : function ( _this, newValue, oldValue, eOpts )  {
										if( Ext.getCmp("loginForm_CHECK_ID_SAVE").checked  ) {
											...
											// Ext.util.Cookies.set('LOGIN_ID', newValue, expiry);
											setCookie('LOGIN_ID', newValue, 7);
										} else {
											// Ext.util.Cookies.set('LOGIN_ID', '');
											setCookie('LOGIN_ID', '', 7);
										}
							...
							},
							{
								...
								inputValue: 'Y',
								anchor: '100%',
								checked:  setCookie('CHECK_ID_SAVE')=="Y" ? true : false, // Ext.util.Cookies.get('CHECK_ID_SAVE') ? true : false,
								listeners: {
									change:	function (cb, newValue, oldValue, eOpts) {
										// Ext.util.Cookies.set('CHECK_ID_SAVE', newValue);
										setCookie('CHECK_ID_SAVE', newValue, 7);
										if( cb.checked ) {
											var expiry = new Date(Date.now() + (7 * 24 * 60 * 60 * 1000));
											// date = date.toUTCString();
											// Ext.util.Cookies.set('LOGIN_ID', Ext.getCmp("loginForm_LOGIN_ID").getValue(), expiry);
											setCookie('LOGIN_ID', Ext.getCmp("loginForm_LOGIN_ID").getValue(), 7);
										} else {
											// Ext.util.Cookies.set('LOGIN_ID', '');
											setCookie('LOGIN_ID', '', 7);
										}
	 ...
	 * 사용자 등록
	 */
	static addUserWindow(_animateTarget){
	 ...
	 * 사용자 수정
	 */
	static editUserWindow(_animateTarget){
		...
		var window = Ext.create('Ext.window.Window', {
			  ...
			  listeners : {
				  afterrender : function( _this, eOpts) {
					 ...
					 Ext.create('Ext.data.Store', {
						 ...
						 listeners : {
							 load : function(_this, _records, _successful, _eOpts ) {
								  Ext.getCmp('userEditForm').loadRecord(_records[0]);
							 }
			  ...
			  buttons : [
				  { text: '저장', handler : function() {
						Ext.getCmp('userEditForm').isValid();
						
						if( Ext.getCmp('userEditPwd_1').getValue() != Ext.getCmp('userEditPwd_2').getValue()) {
								Ext.Msg.alert('WARN', '패스워드가 일치하지 않습니다.', function() {
								  Ext.getCmp('userEditPwd_2').focus();
								});
								return;
						}
						Ext.getCmp('userEditForm').submit({
							msgConfirm : '회원정보를 수정하시겠습니까?',
							clientValidation: true,
							url: '/user/data/update.do',
							success: function(form, action) {
								/*
								var node = Ext.getStore("domainTreeStore").findNode( "DOMAIN_ID", domain_id);
								node.appendChild({"DOMAIN_ID" : "DOMAIN_ID", "DOMAIN_NM" : "DOMAIN_NM", "leaf":false, "text" : "text"});
								*/
								...
								Ext.Msg.alert('성공', action.result.message, function() {
									Ext.getCmp("topEditUser").setText(Ext.getCmp("userEditForm_USR_NM").getValue()+"님의 로그인 정보 변경");
									//Ext.getStore("subjectEntityListStore").reload();
									//window.close();
								});
							...
							failure: function(form, action) {
								switch (action.failureType) {
									...
									case Ext.form.action.Action.SERVER_INVALID:
										Ext.Msg.alert('실패', action.result.errorMessage, function() {
											Ext.getCmp('userEditForm_userEditPwd').focus();
										});
								}
							}
						});
			  ...
			  items : [
				  {
						...
						items: [
							...
							{	
								xtype : 'component',
								html : "* 비밀번호를 변경 하려고 할 경우에는, '변경 패스워드(PWD)'와 '변경 패스워드확인(PWD)'을 입력후 저장하세요.",
								id : "shareProjectIdDesc"
							},
						]
				}

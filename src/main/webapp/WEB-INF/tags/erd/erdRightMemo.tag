						...
						items : [
							{
								...
								items : [
									<tagErd:itemText type="textfield_ux" label="메모" name="WORDS" value="" placeholder="';'로 구분하면 다건조회 가능">
										listeners : {
											'render' : function(cmp) {
									   ...
									   listeners : { 
										   click : function(_this, e, eOpts) { 
											   if( this.up('form').getForm().isValid() ) {
												   ...
												   var store = Ext.getStore("memoListStore").load({page : 1, limit : 999999 , params: me.up('form').getValues(false, false, false, false)}); store.currentPage = 1; store.params = me.up('form').getForm().getFieldValues(false);
											   }
										   }
										}
								  </tagErd:button>
								...
								scrollable:true,
								<tagErd:store type="store" id="memoListStore" idProperty="WORD" url="/memo/data/list.do" rootProperty="data" params="" autoLoad="true">
								...
								tbar : ['->',
									<tagErd:button type="button" label="리로드" iconCls="search" cls="btn_segmentedbutton">
										listeners : {
											click : function(_this, e, eOpts) { 
												var store = Ext.getStore("memoListStore");
												store.reload();
											}
										}
									</tagErd:button>
									<tagErd:button type="button" label="메모 삭제" iconCls="search" id="erdRightMemoDelete" cls="btn_segmentedbutton">
										disabled : ${sessionScope._sessionVO.notModelerRole} ,
										listeners : {
											click : function(_this, e, eOpts) { 
												var grid = Ext.getCmp("rightMemo_gridpanel");
									...
									<tagErd:button type="splitbutton" label="컬럼 추가" iconCls="search" id="erdRightMemoAdd" cls="btn_segmentedbutton">
										disabled : ${sessionScope._sessionVO.notModelerRole} ,
										listeners : {
											click : function(_this, e, eOpts) { 
										...
										menu: {
											...
											disabled : ${sessionScope._sessionVO.notModelerRole} ,
											items: [ 
													{text: '3건 추가', id : 'rightMemo_addColumn3',
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
													...
													{text: '5건 추가', id : 'rightMemo_addColumn5',
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
														handler : function(_this, ev) {
															 var grid = Ext.getCmp("rightMemo_gridpanel");
															 var selectedRecord = grid.getSelectionModel().getSelection()[0];
													...
													{text: '10건 추가', id : 'rightMemo_addColumn10',
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
														handler : function(_this, ev) {
															 var grid = Ext.getCmp("rightMemo_gridpanel");
															 var selectedRecord = grid.getSelectionModel().getSelection()[0];
													...
													{text: '다건 추가', id : 'rightMemo_addColumnMulti',
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
														handler : function(_this, ev) {
													...
													{text: '공통컬럼 추가', id : 'rightMemo_addColumnCommon',
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
														handler : function(_this, ev) {
															 var store = Ext.getStore("memoListStore");
									...
									<tagErd:button type="none" label="사전 적용" iconCls="search" id="erdRightMemoApply" cls="btn_segmentedbutton">
										menu: {
											plain: true,
											items: [ 
													{text: '테이블+컬럼 사전적용', 
														disabled : ${sessionScope._sessionVO.notModelerRole} ,
														handler : function(_this, ev) {
															var gridColumn = Ext.getCmp("rightMemo_gridpanel");
															gridColumn.submit('changed', '/memo/data/save.do', {
																showSaving : true,
																forceSave : true,
									...
									<tagErd:button type="button" label="저장" iconCls="search" id="erdRightMemoSave" cls="btn_segmentedbutton">
										disabled : ${sessionScope._sessionVO.notModelerRole} ,
										listeners : {
											click : function(_this, e, eOpts) { 
												var gridColumn = Ext.getCmp("rightMemo_gridpanel");
								...
								columns: [
									{ xtype: 'rownumberer', },
									{ text: '메모', header: '<div style="text-align:center;width:100%;"><div class="grid-header-textfield"></div>메모</div>', dataIndex: 'WORD', width : 120, minWidth : 100, locked : true, 
										editor: {
											allowBlank: false,
											selectOnFocus: false
										},
									...
									{ text: '메모', header: '<div style="text-align:center;width:100%;"><div class="grid-header-textfield"></div><div class="grid-header-filter"></div>ENGLISH</div>', dataIndex: 'ENGLISH', width : 120, minWidth : 100, locked : false, filter: {type: 'string', dataIndex: 'ENGLISH'},
										editor: {
											allowBlank: true,
											selectOnFocus: false
										},
									},
									{ text: '설명', header: '<div style="text-align:center;width:100%;"><div class="grid-header-textfield"></div><div class="grid-header-filter"></div>설명</div>', dataIndex: 'WORD_DESC', flex : 1, width : 120, minWidth : 100, filter: {type: 'string', dataIndex: 'WORD_DESC'}, cellWrap: true,
										editor :  {

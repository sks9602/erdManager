	...
	$.fn.minimap = function( $mapSource ) {
        ...
        */
		...
		function init() {
			$minimap.find( ".minimap-node" ).remove();
			//creating mini version of the supplied children
			$mapSource.children("svg").children().each( function() {
				var $child = $( this );
				if($child[0].tagName == 'g' ) {
                    // console.log(minimapWidth, minimapHeight, $mapSource[ 0 ].scrollWidth, $mapSource[ 0 ].scrollHeight );
    				var mini = $( "<div></div>" ).addClass( "minimap-node" );
    				$minimap.append( mini );
    				var ratioX = minimapWidth / $mapSource[ 0 ].scrollWidth;
    				var ratioY = minimapHeight / $mapSource[ 0 ].scrollHeight;
                    
                    var tableGrp = SVG($child[0].instance.node);
                    var tableRect = SVG($child[0].instance.node);
                    
    				var wM = tableRect.width() * ratioX; //$child.width() * ratioX;
    				var hM = tableRect.height() * ratioY; //$child.height() * ratioY;
                    
    				var xM = (tableGrp.transform().translateX) * ratioX; ; //($child.position().left + $mapSource.scrollLeft()) * ratioX;
    				var yM = (tableGrp.transform().translateY) * ratioY; ; // ($child.position().top + $mapSource.scrollTop()) * ratioY;

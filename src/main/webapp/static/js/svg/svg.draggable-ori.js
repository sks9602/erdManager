...
(function (svg_js) {
    ...
    _createClass(DragHandler, [{
      ...
      value: function drag(ev) {
		...
		if( this.relationInfo && this.StartOrEnd != "") {
			...
			else if( (mY <= tableGrpBox.top-4  || mY >= tableGrpBox.bottom+1) && this.direction == 'v' ) { //  
      ...
      value: function endDrag(ev) {
		...
		if( this.relationInfo && this.StartOrEnd != "") {
			...
			tableGrpBox = { 
							 ...
							 , bottom : Math.floor(tableGrp.transform().translateY + tableRect.height()) + 12  // DrawTable.CONSTANT.COLUMN_HIGHT
							};
  ...
  svg_js.extend(svg_js.Element, {
    draggable: function draggable(direction, relationInfo, StartOrEnd) {
